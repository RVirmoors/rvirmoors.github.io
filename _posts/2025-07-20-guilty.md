...
![2020-03-13 para long fail](pl_fail.png)
...
This is good enough, I think. It's important to know that that huge drop isn't caused by a single trade, but dozens, with the exposure on that day (just from this strategy) being around 300%... so realistically not all of those trades would've been executed. Also seeing how the same bet works perfectly 3-4 days later, to recover all the losses, I think this strategy does its job.
